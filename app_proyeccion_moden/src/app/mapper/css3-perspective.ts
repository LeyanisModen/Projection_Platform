...
function rep(s: any, v: any, k: any) {
    const n = s[k];
    const ret = Array(n);
    let i;
    if (k === s.length - 1) {
...
function dotMMsmall(x: any, y: any) {
...
function getNormalizationCoefficients(srcPts: any, dstPts: any, isInverse: any) {
    if (isInverse) {
        const tmp = dstPts;
        dstPts = srcPts;
        srcPts = tmp;
...
export default function fixPerspective(srcPts: any, dstPts: any) {
    // console.log(srcPts)
    // console.log(dstPts)
